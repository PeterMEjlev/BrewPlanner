...
def read_inkbird() -> dict[str, float]:
    ...
    """
    if SIMULATE:
        # A setpoint of 18.0C with the temperature drifting +/-0.6C around it on
        # a ~20-minute cycle; the relays mimic a real hysteresis controller.
        setpoint = 18.0
    ...
    # Imported lazily so SIMULATE mode needs no pip install.
    import tinytuya
    ...
    if not (INKBIRD_DEVICE_ID and INKBIRD_IP and INKBIRD_LOCAL_KEY):
        raise RuntimeError(
            ...
            "for live reads. Run the tinytuya wizard (see README) or set "
